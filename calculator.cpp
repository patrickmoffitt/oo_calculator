...
/*
 * The intent of the format_* functions is to transform the input equation into a proper white-space delimited string
 * that can be trivially tokenized into the operator and value stacks. Pasting from the Mac OS Numbers application or
 * Microsoft Excel is supported.
 *
...
// The Mac OS Numbers application uses an alternate division character. This function replaces it with '/'.
...
// The Mac OS Numbers application uses an alternate multiplication character. This function replaces it with '*'.
...
/*
 * Finds sequences of 3 or more operators, and doubled * or /. Also finds more than one operator between sets of
 * parentheses. The algorithm gets this wrong so its excluded. The Mac OS Numbers application and MS Excel get these
 * right so these's room for improvement.
 */
...
bool Infix_calculator::validate_operator_sequence() {
    ...
    if (regex_search(equation, rgx_excess_operators)) {
        cerr << endl << "Detected invalid sequence of arithmetic operators in: " << equation << endl;
        result = true;
    }
    return result;
}
...
void Infix_calculator::calculate() {
    ...
    if (int(value_stack.size()) > 1) {
        ...
        std::cerr << "Equation lacks sufficient values." << std::endl;
        auto err = std::make_error_condition(std::errc::operation_canceled);
        throw std::runtime_error(err.message());
    }
    // 3 Apply the operator to the operands, in the correct order.
    std::cout << "operator " << operator_ << ", operand_l " << operand_l << ", operand " << operand << std::endl;
    result = Infix_calculator::operators[operator_](operand_l, operand);
    // 4 Push the result onto the value stack.
    value_stack.emplace_front(result);
    std::cout << "Pushing " << result << " onto value stack. Size = " << int(value_stack.size()) << std::endl;
}
...
// Public
Infix_calculator::Infix_calculator(string equation) {
...
double Infix_calculator::solve() {
    ...
    equation = format();
    if (not validate()) {
        cout << endl << equation << " Looks better than " << input << endl << endl;
        ...
        for (sregex_iterator it = tokens_begin; it != tokens_end; ++it) {
            ...
            if (is_double(token)) {
                ...
                cout << "Pushing " << token << " onto value stack. Size = " << int(value_stack.size()) << endl;

                // 1.2.2 A variable: get its value, and push onto the value stack.
                // 1.2.3 A left parenthesis: push it onto the operator stack.
            } else if (*token.c_str() == *lp) {
                operator_stack.emplace_front(*token.c_str());
                cout << "Pushing " << token << " onto operator stack." << endl;
                ...
                if (search != operators.end()) {
                    ...
                    cout << "Token " << operator_ << " is an operator. Size " << int(operator_stack.size()) << endl;
                    ...
                    while (int(operator_stack.size()) > 0 and
                           ...
                           operator_precedence[operator_stack.front()] <= operator_precedence[operator_] and
                           operator_stack.front() != *lp) {
                        calculate();
                    }
                    // 2 Push operator_ onto the operator stack.
                    operator_stack.emplace_front(operator_);
                    cout << "Pushing " << operator_ << " onto the operator stack. Size " << int(operator_stack.size())
                         << endl;
                }
            }
        }
